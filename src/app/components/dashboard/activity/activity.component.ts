...
export class ActivityComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.activityService.addListener((activities: ActivityModel[]) => {
      console.log('Activities received', activities);
      this.activities = activities;
      this.isLoading = false;
    });

    // Vérification de la connection API

    try {
      const activities = await this.activityService.getAllActivities();
      console.log('Activities fetched from API', activities);
      this.activities = activities;
    } catch (error) {
    ...
    if (this.isFormEdition) {
      ...
      try {
        ...
        const updatedActivity: ActivityModel = {
          ...response,
          upVotes: this.activityToEdit.upVotes,
          downVotes: this.activityToEdit.downVotes,
          positiveFeedback: this.activityToEdit.positiveFeedback
        };
        this.activities.push(updatedActivity);
  ...
  emitActivity(): void {  
    ...
    this.activityService.emitEvent(activity);
  }
  ...
  public async updateActivity(activity: any,activityUpdated: ActivityEditionModel): Promise<void> {
    ...
    try {
      const response: ActivityEditionModel = await this.activityService.updateActivity({
        ...activityUpdated,
      }).toPromise();
      // Finding the activity to update in the list
      const index = this.activities.findIndex((a) => a.activity_Id === response.activity_Id);
      if (index !== -1) {
        this.activities[index] = { ...this.activities[index], ...response }; //Updates existing activity
      } else {
        const updatedActivity: ActivityModel = {
          ...response,
          upVotes: this.activityToEdit?.upVotes,
